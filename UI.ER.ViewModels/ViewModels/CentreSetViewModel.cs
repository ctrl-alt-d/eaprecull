...
namespace UI.ER.ViewModels.ViewModels
{
    ...
    public class CentreSetViewModel : ViewModelBase, ISetViewModel<CentreCreateViewModel, Dtoo.Centre>
    {
        ...
        public CentreSetViewModel(IServiceFactory serveis, bool modeLookup = false)
        {
            ...
            {
                var update = new CentreCreateViewModel(_serveis);

                var data = await ShowDialog.Handle(update);

                if (data != null)
                {
                    var item = new CentreRowViewModel(_serveis, data, ModeLookup);
        ...
        private async Task OmplirAmbElsNousValors(bool nomesActius)
        {
            ...
            // Petició al backend            
            using var bl = _serveis.GetBLOperation<ICentreSetAmbActuacions>();
            ...
            var newItems =
                ...
                .Select(x => new CentreRowViewModel(_serveis, x, ModeLookup));
